...
class CustomerController extends Controller
{
    ...
    public function submit_change_password(Request $request)
    {
        ...
        if ($customer->password == md5($data['password'])) {
            ...
            echo $output = 'Nhập mật khẩu cũ không đúng';
    }
    //Form thêm địa chỉ
    public function form_address()
    {
        ...
        return view("shop.customer.my-address")->with(compact('list_category', 'list_brand', 'customer','provinces'));
    }
    //load tỉnh/huyện/xã
    ...
    public function insert_address(Request $request)
    {
        ...
        $address->idProvince = $data['idProvince']; // Lưu ID tỉnh
        $address->idDistrict = $data['idDistrict']; // Lưu ID huyện
        $address->idWard = $data['idWard'];
    ...
    public function edit_address(Request $request, $idAddress)
    {
        ...
        $address->PhoneNumber = $data['PhoneNumber'];
        $address->Address = $data['Address'];
        $address->idProvince = $data['idProvince'];
        $address->idDistrict = $data['idDistrict'];
        $address->idWard = $data['idWard'];
        $address->save();
    }
...
//     public function fetch_address()
// {
//     $list_address = AddressCustomer::with(['province', 'district', 'ward'])
...
//     foreach ($list_address as $key => $address) {
//         $provinceName = $address->province ? $address->province->name : 'N/A';
//         $districtName = $address->district ? $address->district->name : 'N/A';
//         $wardName = $address->ward ? $address->ward->name : 'N/A';

//         $output .= '<li class="cus-radio align-items-center justify-content-between">
//                         <input type="radio" name="address_rdo" value="' . $address->idAddress . '" id="radio' . $address->idAddress . '" checked>
//                         <label style="width: 80%; line-height: 1.5" class="mb-30" for="radio' . $address->idAddress . '">
//                             <span>' . $address->CustomerName . '</span>
//                             <span>' . $address->PhoneNumber . '</span>
//                             <span>' . $address->Address . ', ' . $wardName . ', ' . $districtName . ', ' . $provinceName . '</span>
//                         </label>
//                         <div class="mt-0">
//                             <button type="button" data-toggle="modal" data-target="#EditAddressModal" class="edit-address btn btn-outline-primary" data-id="' . $address->idAddress . '" data-name="' . $address->CustomerName . '" data-phone="' . $address->PhoneNumber . '" data-address="' . $address->Address . '">Sửa</button>
//                             <button type="button" class="dlt-address btn btn-outline-primary ml-2" data-id="' . $address->idAddress . '">Xóa</button>
...
public function fetch_address()
{
    ...
    foreach ($list_address as $key => $address) {
        ...
        // Lấy ID của tỉnh, huyện, xã nếu có, nếu không thì đặt là null
        $provinceId = $address->province ? $address->province->idProvince : null;
        $districtId = $address->district ? $address->district->idDistrict : null;
        $wardId = $address->ward ? $address->ward->idWard : null;
...
// die();
        ...
        if (isset($_GET['sort_by'])) {
            if ($_GET['sort_by'] == 'new')
                $list_pd_query->orderBy('created_at', 'desc');
            else if ($_GET['sort_by'] == 'old')
                $list_pd_query->orderBy('created_at', 'asc');
            else if ($_GET['sort_by'] == 'bestsellers')
                $list_pd_query->orderBy('Sold', 'desc');
            else if ($_GET['sort_by'] == 'featured')
                $list_pd_query->whereBetween('product.created_at', [$sub30days, now()])->orderBy('Sold', 'desc');
            else if ($_GET['sort_by'] == 'sale')
                $list_pd_query->join('saleproduct', 'saleproduct.idProduct', '=', 'product.idProduct')->whereRaw('SaleStart < NOW()')->whereRaw('SaleEnd > NOW()')->orderBy('created_at', 'desc');
            else if ($_GET['sort_by'] == 'price_desc')
                $list_pd_query->orderBy('Price', 'desc');
            else if ($_GET['sort_by'] == 'price_asc')
